...
- `update` syntax payload is scope-interned: the payload carries a `scopes` table once, and each line carries flat `[start, end, scope_id, ...]` triples under `spans`. Backend owns the encoding; frontends decode it and must fail closed on misaligned triples, inverted ranges, out-of-order (overlapping) triples, span starts past the served line, or unknown scope ids, and clip a span end to the line length. Spans of one scope share a single interned string per payload. A rejected payload must leave the frontend's caches untouched so the row can be re-requested.
- `update` line text has two carriers, chosen by transport capability, never by frontend version. A row carries either `text` (a JSON string) or its bytes in the raw binary frame that immediately follows the notification; an update may mix them. The frame itself is `varint row_count || varint len_i... || row bytes`, and an op declares its rows come from it with `"blob": true`, so a row without `text` consumes the next frame row in op order. Only transports that report `supports_binary_frames` (the in-process channel pair today) are eligible, so newline-framed and plugin stdio peers keep receiving per-line JSON. The frame is backend-owned data: frontends decode it into their cache and must reject the payload (cache untouched, error surfaced) when an op declares a frame that did not arrive, when the frame's rows and the payload's references disagree, when a row is not UTF-8, or when a blob-op row has neither `text` nor a frame row.
