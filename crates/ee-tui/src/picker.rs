...
impl PickerState {
    ...
    pub(crate) fn new_files(cwd: PathBuf) -> Self {
        ...
        Self { kind: PickerKind::Files, query: String::new(), cwd, items, filtered, selected: 0 }
    }
    ...
    pub(crate) fn new_grep(initial_query: String, cwd: PathBuf) -> Self {
        let items = if initial_query.is_empty() {
            Vec::new()
        } else {
            grep_files(&initial_query, &cwd)
        };
        let filtered = (0..items.len()).collect();
        Self {
    ...
    fn refresh(&mut self) {
        if matches!(self.kind, PickerKind::LiveGrep) {
            ...
            self.items = if self.query.is_empty() {
                ...
                grep_files(&self.query, &self.cwd)
            };
...
fn collect_files(cwd: &Path) -> Vec<PickerItem> {
    ...
    let walker =
        WalkBuilder::new(cwd).hidden(false).git_ignore(true).max_depth(Some(10)).build();
    for entry in walker.flatten() {
        ...
        let label = path
            ...
            .to_string_lossy()
            .into_owned();
        items.push(PickerItem { label, detail: None, path: Some(path), buf_id: None, line: None });
...
mod tests {
    ...
    fn picker_navigation_clamps() {
        let bufs = vec![
            (1u32, "a.rs".to_owned(), None),
            (2u32, "b.rs".to_owned(), None),
        ];
        let mut picker = PickerState::new_buffers(bufs);
    ...
    fn picker_pop_char_restores_filter() {
        let bufs = vec![
            (1u32, "foo.rs".to_owned(), None),
            (2u32, "bar.rs".to_owned(), None),
        ];
        let mut picker = PickerState::new_buffers(bufs);
