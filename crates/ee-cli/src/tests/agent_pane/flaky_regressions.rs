...
pub(super) fn phase_six_live_openrouter_pane_resume_reuses_completed_write() {
    let _live_lock = phase_six_live_lock();
    let workspace = tempfile::tempdir().expect("fixture workspace");
    ...
    assert_eq!(pending.info.completed_tool_calls, 1, "checkpoint must retain completed write");
    let evidence_before_resume = thread
        .terminal_evidence
        .as_ref()
    ...
    assert_eq!(
        scripted.request_bodies().len(),
        2,
        "virtual deadline expires only after pending model request starts"
    );
    ...
    assert!(
        bodies[3]["messages"]
            ...
            .any(|message| message["role"] == "tool"
                && message["tool_call_id"] == "write-after-resume"),
