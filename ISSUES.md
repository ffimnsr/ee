...
## OpenCode Go and Zen ACP Agent Plan

Add an `ee-opencode-agent` ACP stdio binary backed by `ee-agent-orchestrator`. It must support OpenCode Zen and OpenCode Go as separate service surfaces, selecting protocol from exact `(surface, model)` routing data rather than assuming every model speaks one OpenAI-shaped API.

Sources reviewed: [OpenCode Go](https://opencode.ai/docs/go/) and [OpenCode Zen](https://opencode.ai/docs/zen/) (2026-08-24).

Current documented endpoint roots:

- Zen: `https://opencode.ai/zen/v1`.
- Go: `https://opencode.ai/zen/go/v1`.
- OpenAI Responses dialect: `/responses`.
- Anthropic Messages dialect: `/messages`.
- OpenAI-compatible Chat Completions dialect: `/chat/completions`.
- Model metadata lists live at `/models` under each root; metadata discovery must not silently override an unrecognized model's protocol routing.

Rules:

- [ ] Use `OPENCODE_API_KEY` only for `Authorization: Bearer`; never log, persist, include in setup output, transcript, checkpoint, diagnostic, or error detail.
- [ ] Require explicit `OPENCODE_SURFACE=zen|go` and `OPENCODE_MODEL`; do not choose a billable model, surface, or protocol by an implicit default.
- [ ] Route only an exact supported `(surface, model)` catalog entry. Unknown, retired, and mismatched entries fail closed before any request writes credentials.
- [ ] Keep endpoint construction internal: no unrestricted `OPENCODE_API_URL` override that could disclose an OpenCode credential to an arbitrary origin. Test transport injection remains available without environment endpoint overrides.
- [ ] Reuse `ee-acp-agent-server`, `ee-agent-orchestrator`, existing MCP bridge, trust policy, recovery, cancellation, streaming, and setup-manifest conventions. Do not add a second ACP loop, tool executor, session store, or simple provider mode.
- [ ] Consolidate generic OpenAI Chat Completions behavior now shared by OpenRouter and OpenCode. Provider-specific headers and response quirks stay outside shared code.
- [ ] Treat OpenAI Responses, Anthropic Messages, and OpenAI-compatible Chat Completions as distinct request/response/streaming codecs. Never route by model-name prefix or attempt one dialect after another with same request.
- [ ] Exclude currently documented Google Generative Language endpoint models from this plan. Surface clear unsupported-model guidance; add Google dialect only through separate protocol and security review.

### Phase 1: Model-surface contract and exact routing catalog

Goal: define stable, auditable OpenCode surface/model routing before HTTP implementation.
...
Goal: remove duplicated chat-completions transport and normalized-orchestrator mapping before adding OpenCode codecs.
...
Goal: implement protocol-correct OpenAI Responses and Anthropic Messages adapters against normalized orchestrator types.
...
Goal: expose one production ACP agent which selects correct OpenCode adapter from safe route/config resolution.
...
Goal: make provider setup understandable while keeping live catalog changes safe.
...
Goal: prove route selection, codecs, agent integration, and extracted shared code remain correct without live paid API traffic.
...
- [x] Expose configured ACP agent selection when creating a thread; current `:agents` and `:agents_new` always resolve `agents.default_agent`.
...
- [x] Extend `:agents_new` with optional agent id.
  - [x] Accept `:agents_new <agent_id>` and start a session through existing `AgentManager::new_session(agent_id, ...)`.
  - [x] Preserve no-argument behavior when exactly one agent exists or `agents.default_agent` resolves.
  - [x] Open an agent picker when multiple agents exist and no unambiguous default is available.
  - [x] Reject unknown ids with a bounded message listing configured ids.
- [x] Extend local composer `/new` with optional agent id using the same resolver and picker.
- [x] Add a new-thread agent picker.
  - [x] List configured agent id, configured display label when available, default marker, and local connection state.
  - [x] Do not initialize or spawn unselected agents to populate the picker.
  - [x] Keep launch command and environment values hidden or redacted.
- [x] Show owning agent id in thread picker, thread status, exported transcript metadata, recovery entries, and failure diagnostics.
- [x] Keep thread switching independent of agent connection; selecting an existing thread must reconnect only its owning agent when needed.
- [x] Add tests with two fake ACP agents proving each selected thread sends `initialize` and `session/new` only to its selected process.
- [x] Add tests proving approvals, terminals, evidence, session load/resume, and shutdown remain scoped by agent id plus session id.
- [x] Document multiple manual agent entries under `[agents.servers.<id>]` and `agents.default_agent`.

#### Exit criteria

- [x] User can create and switch between threads owned by different configured ACP agents without editing config between sessions.
- [x] Selecting one agent never starts, authenticates, or sends workspace context to another agent.
- [x] Existing single-agent `:agents`, `:agents_new`, and `/new` behavior remains unchanged when selection is unambiguous.
...
- [x] Add typed model identity metadata to `ModelRegistry` entries.
  - [x] Record stable model id, provider id, declared model family, display name, and bounded capability hints.
  - [x] Add a non-secret `ModelFamily` type covering known families plus explicit `other` identity.
  - [x] Reject empty ids, duplicate ids, and malformed provider/family metadata.
  - [x] Never infer a trustworthy contrast solely from display name or free-form capability text.
- [x] Add deterministic contrasting-model selection.
  - [x] Exclude the active model id.
  - [x] Exclude models from the active model family.
  - [x] Require chat completion and required tool capabilities.
  - [x] Prefer an explicit `rubber_duck` role route, then strong tier, then stable route/model id order.
  - [x] Return typed unavailable reasons for unknown active identity, no alternative, same-family-only alternatives, missing capability, and disabled route.
  - [x] Never fall back to the active model or same-family model.
- [x] Refactor `OrchestratorProvider` production construction to own a shared `ModelRegistry` rather than one generic model adapter.
  - [x] Require a registered `default` adapter before provider startup succeeds.
  - [x] Create new and restored session runtimes through `OrchestratorRuntime::with_model_registry` or equivalent.
  - [x] Restore checkpoints and normal session state against process-owned adapters; never serialize adapters, credentials, or HTTP clients.
  - [x] Keep a one-adapter constructor only as a focused convenience that builds a single-entry registry.
- [x] Extend `ee-openrouter-agent` configuration with optional critic model and family metadata.
  - [x] Keep `OPENROUTER_MODEL` as root/default model.
  - [x] Add explicit rubber duck model configuration without reusing or exposing the API key in model metadata.
  - [x] Build separate adapters for root and critic model ids while sharing only safe immutable HTTP/provider configuration.
  - [x] Reject same-id or same-family critic configuration with a clear startup diagnostic; continue root-agent operation with critic unavailable.
...
- [x] Add unit and provider-flow tests for registry construction, family contrast, deterministic selection, session load/resume, and single-model degradation.

#### Exit criteria

- [x] Production ee-owned agent can run root and critic calls on distinct registered model adapters.
- [x] Every critic invocation records active and selected model ids while proving different declared families.
- [x] Single-model setups remain usable and report rubber duck unavailable instead of failing ordinary turns.
...
- [x] Add `BuiltinSubagentRole::RubberDuck` in `ee-agent-orchestrator`.
  - [x] Allow `SideEffectClass::Read` only.
  - [x] Deny `Write`, `Execute`, `Delegate`, terminal lifecycle, mutating code actions, and approval-producing operations.
  - [x] Cap iterations, context bytes, output bytes, timeout, and recursion below root defaults.
  - [x] Require citations and quarantine failed, cancelled, malformed, or unverified output.
- [x] Add typed critique targets: plan, implementation, tests, failure analysis, and explicit user question.
- [x] Add versioned `CritiqueReport` and `CritiqueFinding` schemas.
  - [x] Classify findings as blocking, non-blocking, or suggestion.
  - [x] Require stable key, issue, impact, concrete recommended change, confidence, and cited evidence for each finding.
  - [x] Accept an explicit empty finding list as a clean review.
  - [x] Bound finding count and every text/evidence field.
  - [x] Reject unknown schema versions, duplicate keys, unsupported severities, empty substantive fields, and uncited observed claims.
- [x] Reuse `SubagentReportVerifier`, execution evidence, citation sinks, and quarantine where their contracts match.
- [x] Extract shared bounded review-context assembly from `reflection.rs` instead of duplicating changed-file, validation, task, diagnostic, and revision collection.
- [x] Keep repository and tool content marked untrusted and pass critic requests through existing prompt-injection preparation.
- [x] Add parser/verifier tests for valid report, clean report, malformed JSON, oversized output, duplicate findings, absent citations, and injected repository instructions.

#### Exit criteria

- [x] Rubber duck can only inspect bounded read context and return a verified structured report.
- [x] Critic has no code path that applies edits, runs commands, delegates, or requests approval.
- [x] Root agent receives findings as evidence, not permission to mutate or proof of completion.
...
- [x] Add `RubberDuckRunner` or equivalent orchestrator service.
  - [x] Accept target, user goal, active task/plan, active model identity, optional user question, current revision, and bounded observed evidence.
  - [x] Select critic through deterministic contrasting-model API before creating a child task.
  - [x] Reserve model/subagent budget before dispatch; budget denial must not create task state.
  - [x] Inherit root cancellation and use dedicated bounded timeout.
  - [x] Filter tool discovery to read-only definitions and apply a second fail-closed read-only policy.
  - [x] Verify report before merging any summary into root transcript or memory.
- [x] Add typed outcomes: completed, unavailable/skipped, quarantined, cancelled, and failed.
- [x] Inject verified report into root transcript as bounded critic evidence.
  - [x] Do not expose hidden reasoning or raw critic transport output.
  - [x] Let root summarize accepted findings and planned changes to user timeline.
  - [x] Keep critic prose incapable of overriding host validation or completion evidence.
- [x] Add root finding resolution states: accepted, rejected with cited evidence, and deferred with reason.
- [x] Convert accepted blocking/non-blocking findings into task-graph items only through root-owned reconciliation.
- [x] Prevent `verified` completion while a current-revision blocking finding remains unresolved.
- [x] Cache only by session, target, active/root model ids, revision, plan/task fingerprint, and policy version; invalidate after writes, model switch, diagnostics, validation, graph, or checkout changes.
- [x] Add hermetic tests proving contrasting adapter use, read-only policy, cancellation, unavailable critic degradation, report injection, and unresolved-blocking completion behavior.

#### Exit criteria

- [x] Same ee-owned ACP agent can use one model for root work and another model family for rubber duck critique without switching root session model.
- [x] Root remains sole writer and final decision owner.
- [x] Critic failure or absence never corrupts session state or silently substitutes same-model reflection.
...
- [x] Add shared `/rubber-duck [question]` slash command parsing and advertisement.
  - [x] Match exact command name; reject prefix collisions such as `/rubber-ducking`.
  - [x] Preserve optional instruction text exactly after separator normalization.
  - [x] Advertise command only when provider supports rubber duck flow; unavailable configured critic may still return a typed reason.
- [x] Route manual command before normal mutable MCP/tool registration, like `/compact` special handling.
- [x] Run critic, inject verified report, then let root perform one bounded synthesis response.
- [x] Emit concise timeline updates: critic selected, skipped reason, finding counts, and root plan change. Do not dump raw critic report unless user explicitly requests details.
- [x] Add deterministic automatic trigger policy with manual-only default until replay gate passes.
  - [x] Plan trigger: after `PlanThenExecute` plan/task emission and before first write for multi-file, API, security, persistence, migration, destructive, or high-coupling work.
  - [x] Implementation trigger: after writes and focused validation, before final completion synthesis, when changed scope, diagnostics, skipped validation, recovery, or public API impact is non-trivial.
  - [x] Failure trigger: after bounded repeated edit/tool/validation/no-progress evidence, before another equivalent repair attempt.
  - [x] Test trigger: when changed behavior lacks adjacent selected tests or validation remains partial.
- [x] Skip automatic critique for pure questions, summaries, formatting-only work, one-file mechanical edits with focused passing evidence, prior same-target/current-revision critique, cancellation, unavailable contrast, or exhausted budget.
- [x] Run at most once per target/revision/trigger key unless root plan or workspace revision materially changes.
- [x] Add trigger tests proving pre-write ordering, post-write revision binding, repeated-failure single invocation, trivial-task skip, and no hidden unlimited loops.

#### Exit criteria

- [x] User can request a second opinion with `/rubber-duck` and optional focused question.
- [x] Automatic critique occurs only at deterministic high-leverage boundaries and never after unbounded speculative retries.
- [x] Root reports how critique changed or did not change plan without treating critic opinion as validation evidence.
...
- [x] Add host-local critic backend selection: internal model or external agent id.
- [x] Add `CriticAgentBroker` in `ee-agent-host` using existing `AgentManager` connections and session APIs.
  - [x] Resolve configured critic agent id without changing `agents.default_agent`.
  - [x] Reject root agent id when configuration requires independent agent implementation.
  - [x] Create an ephemeral critic session with bounded cwd/additional directories and no mutable forwarded tools.
  - [x] Send a bounded critique request containing explicit untrusted context delimiters and structured response schema.
  - [x] Collect final response, verify report, close ephemeral session, and retain only bounded verified evidence.
  - [x] Propagate root cancellation and close orphaned critic session/connection work deterministically.
- [x] Forward only read-class ee MCP tools to critic session.
  - [x] Omit write, execute, terminal, code-action apply, rename apply, note persistence, and approval-producing tools from discovery.
  - [x] Deny any unexpected mutation request at host policy even if critic cached an old manifest.
- [x] Add explicit external-agent read-only trust classification.
  - [x] Distinguish `host_forwarded_read_only` from `sandbox_enforced_read_only`.
  - [x] Never claim arbitrary external agent is fully read-only when its native tools remain outside ee control.
  - [x] Require sandbox-enforced read-only workspace/snapshot or agent-advertised verifiable read-only mode before automatic use.
  - [x] Keep unsandboxed external critic manual-only with visible warning.
- [x] Prevent credential/config sharing between root and critic agents.
  - [x] Each process uses only its configured env/auth flow.
  - [x] Never copy API keys, login tokens, provider config, or native agent state across processes.
- [x] Add two-fake-agent integration tests proving root/critic process separation, context bounds, no mutation forwarding, cancellation, session cleanup, malformed report quarantine, and root synthesis.
- [x] Add adversarial test where external critic attempts native/forwarded write; forwarded request must fail and automatic mode must reject non-sandboxed configuration.

#### Exit criteria

- [x] ee can use same ACP implementation under another configured id/model or a different ACP implementation as manual critic.
- [x] Cross-agent sessions, auth, tools, evidence, costs, and failures remain isolated and attributable.
- [x] Automatic cross-agent rubber duck never runs without enforceable read-only guarantees.
...
- [x] Add `RubberDuckConfig` with off, manual, and automatic modes plus bounded calls, context bytes, output bytes, and timeout.
- [x] Add backend selection configuration for internal model id or external agent id; reject ambiguous simultaneous selections.
- [x] Validate model/agent ids during resolved config construction while allowing ordinary agent operation when optional critic is unavailable.
- [x] Add typed orchestrator/host events for critic started, completed, skipped, quarantined, cancelled, and finding resolution.
  - [x] Record only target, safe reason, model/agent ids, finding counts, latency, token/cost counters, and policy version.
  - [x] Never record raw prompts, critique text, workspace content, paths, terminal output, credentials, or hidden reasoning in telemetry.
- [x] Surface provider/agent identity, extra model call, latency, and estimated cost before or during manual cross-provider critique.
- [x] Attribute internal critics by provider/model/prompt/routing versions and external critics by configured agent id plus advertised implementation metadata.
- [x] Keep critique report out of completion evidence. Only root actions plus host-observed diagnostics/diff/validation may change terminal completion state.
- [x] Add security tests for prompt injection, path escape, symlink escape, stale revision, cached mutable manifest, secret-like content, oversized context/report, cross-agent approval reuse, and terminal ownership.
- [x] Add serialization/roundtrip tests for all new config, events, typed skip reasons, report schema, and resolution states.
- [x] Document privacy and billing boundary: external ACP agents own native auth, provider relationship, retention, model choice, and native tool behavior.

#### Exit criteria

- [x] User controls whether critique runs and which trusted backend it may use.
- [x] Critique cost, latency, provider/agent identity, and skip/failure state are visible without leaking workspace data.
- [x] Critic output cannot grant trust, approval, completion, or cross-agent policy reuse.
...
- [x] Add versioned replay fixtures for flawed plan, missing authorization, stale revision, unsafe migration, missing regression tests, clean mechanical edit, false-positive critique, unavailable critic, timeout, malformed report, and repository prompt injection.
- [x] Add cross-agent fixtures for same implementation/different startup model and different ACP implementations.
- [x] Record quality and cost metrics: useful findings, accepted/rejected/deferred findings, duplicate work, false positives, policy violations, model/agent calls, latency, tokens, estimated cost, and final validation score.
- [x] Define automatic-mode regression thresholds before changing defaults.
  - [x] Require measurable quality improvement on complex fixtures.
  - [x] Require no write/execute/delegate critic calls and no approval prompts.
  - [x] Require trivial fixtures to skip at high rate.
  - [x] Bound latency, token, cost, and duplicate-work regressions.
  - [x] Require no false-success or completion-evidence regression.
- [x] Start rollout with manual internal-model critique.
- [x] Enable automatic internal critique only after replay baseline passes.
- [x] Keep external-agent critique manual until sandbox/read-only contract and adversarial fixtures pass.
- [x] Document examples for:
  - [x] multiple ACP agents with `:agents_new <agent_id>` and picker
  - [x] same ACP binary configured twice with different model arguments/env
  - [x] one ee-owned OpenRouter agent with root and critic model ids
  - [x] different external ACP root and critic agents
  - [x] per-session model config through `:agents_config_set`
  - [x] `/rubber-duck` manual invocation, automatic trigger behavior, skip reasons, privacy, and cost
- [x] Run `cargo fmt --all -- --check`.
- [x] Run `cargo clippy --workspace --all-targets --all-features`.
- [x] Run targeted quiet tests for `ee-agent-protocol`, `ee-agent-host`, `ee-agent-orchestrator`, `ee-openrouter-agent`, and `ee-cli` agents UI.
- [x] Run `./scripts/test-workspace-summary.sh`.

#### Exit criteria

- [x] Multiple ACP agents are selectable per new thread with lazy isolated processes and agent-scoped sessions.
- [x] ee-owned rubber duck uses a verified contrasting model and enforced read-only tools.
- [x] Optional external ACP critic is isolated, bounded, attributable, and never misrepresented as fully read-only without sandbox proof.
