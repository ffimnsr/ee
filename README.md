...
Each replay records a stable SHA-256 trace id, redacted relative workspace snapshot, JSONL event trace, task/validation/recovery/policy score, diff and approval counts, model/tool calls, deterministic latency, tokens, integer micro-USD cost estimate, and counter-only role delegation effectiveness (`useful_findings`, duplicate work, write conflicts, latency, cost). Candidate labels include provider/model, prompt, routing, and `stdio` or ACP-native MCP transport so evidence cannot mix configurations.
